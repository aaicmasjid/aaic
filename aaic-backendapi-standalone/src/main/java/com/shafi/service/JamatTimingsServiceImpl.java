...
@Service
public class JamatTimingsServiceImpl {

	Date date = new Date();
	LocalDateTime now = LocalDateTime.now();

	String cstCurrentDateTime = formatDateToString(date, "dd MM yyyy HH:mm:ss a", "CST");
	String ampm = formatDateToString(date, "a", "CST");
	String currDate = formatDateToString(date, "dd", "CST");
	String currMonth = formatDateToString(date, "MM", "CST");
	String currHour = formatDateToString(date, "HH", "CST");
	String currMunutes = formatDateToString(date, "mm", "CST");
	String currHourMunutes = formatDateToString(date, "hh:mm", "CST");
	DayOfWeek dow = now.getDayOfWeek();
	String today = dow.toString();

	char [] chartDate = new char[cstCurrentDateTime.length()];
	...
	Integer intDate =  Integer.valueOf(currDate);
	Integer intMonth = Integer.valueOf(currMonth);
	Integer intHour = Integer.valueOf(currHour);
	Integer intMinutes = Integer.valueOf(currMunutes);
	...
	//Fajr jamat
	public String yearlyFajrJamatTimings() {
	...
	//Asr jamat
	public String yearlyAsrJamatTimings() {
		...
		}if((intMonth == 3) && (intDate >= marchDaylightSavingDay) && (intDate <= 23) ) {
		...
		if((intMonth == 11) && (intDate < novDaylightSavingDay)) {
			response.setAsrJamat("4:45 PM");
		}if((intMonth == 11) && (intDate >= novDaylightSavingDay) && (intDate <= 15)) {
			response.setAsrJamat("3:30 PM");
		}if((intMonth == 11) && (intDate > 15) && (intDate <= 19)) {
			response.setAsrJamat("3:15 PM");
		}if((intMonth == 11) && (intDate > 19) && (intDate <= 30)) {
			response.setAsrJamat("3:00 PM");
		}
		...
		if(((intMonth >= 12) && ((intDate >= 1)) || ((intMonth <= 3) && (intDate <= 8)))) {
	...
	//Maghrib jamat
	public String yearlyMaghribJamatTimings() {
		...
		//response.setMaghribJamat(response.getMaghrib());
	...
	//Isha jamat
	public String yearlyIshaJamatTimings() {
		...
		if((intMonth == 3) && (intDate < marchDaylightSavingDay) ) {
			response.setIshaJamat("7:30 PM");
		}if((intMonth == 3) && (intDate >= marchDaylightSavingDay) && (intDate <= 22) ) {
		...
		if((intMonth == 11) && (intDate < novDaylightSavingDay)) {
			response.setIshaJamat("7:45 PM");
		}if((intMonth == 11) && (intDate >= novDaylightSavingDay) && (intDate <= 30)) {
			response.setIshaJamat("7:30 PM");
		}
	...
	public String nextPrayer() {
		...
		//Next Fajr
		String fajr =  yearlyFajrJamatTimings(); //prayerTimes.get(0); //05:45 am
